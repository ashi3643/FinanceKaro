                ...
                <div className={`flex-1 rounded-2xl border transition-all duration-300 overflow-hidden ${isUnlocked ? 'bg-surface border-border hover:border-text/30 ' + level.shadow : 'bg-surface/50 border-border/50 opacity-60'}`}>
                  ...
                  {isUnlocked && (
                    <div className="flex w-full divide-x divide-border/50 border-t border-border/50">
                      <Link href={`${basePath}/learn/${level.id}/lesson${isCompleted ? 1 : levelCompletedLessons + 1}`} className={`flex-1 py-3 px-4 flex items-center justify-center text-xs font-bold transition-colors hover:bg-black/5`}>
                        {isCompleted ? (
